import { parseUrl } from './url-template.utils';

describe('Parse Templated Urls', () => {
  it('should parse an un-templated url', () => {
    expect(parseUrl('/api/v1/users')).toEqual('/api/v1/users');
  });

  it('should disregard params if not templated', () => {
    expect(parseUrl('/api/v1/users', { id: 'userid' })).toBe('/api/v1/users');
  });

  it('should disregard params if not needed', () => {
    expect(
      parseUrl('/api/v1/users/{userId}', {
        userId: 'userid',
        extra: 'extra',
      }),
    ).not.toContain('extra');
  ...
  it('should return templates of params not available on parsing', () => {
    expect(parseUrl('/api/v1/users/{userId}/posts/{postId}', { userId: 'userId' })).toBe('/api/v1/users/userId/posts/');
  });

  it('should return parsed templated href', () => {
    expect(parseUrl('/api/v1/users/{userId}', { userId: 'userId' })).toBe('/api/v1/users/userId');
  });

  it('should return parsed templated href with a query parameter', () => {
    expect(parseUrl('/api/v1/users/{userId}{?locale}', { userId: 'userId', locale: 'en' })).toBe(
      '/api/v1/users/userId?locale=en',
    );
  });

  it('should return parsed templated href with multiple query parameters', () => {
    expect(parseUrl('/api/v1/users/{?locale,status}', { userId: 'userId', locale: 'en', status: 'active' })).toBe(
      '/api/v1/users/?locale=en&status=active',
    );
  });

  it('should return parsed templated href with nested query parameters', () => {
    expect(parseUrl('/api/v1/users{?locale,filters*}', { filters: { locale: 'en', status: 'active' } })).toBe(
      '/api/v1/users?locale=en&status=active',
    );
  });
