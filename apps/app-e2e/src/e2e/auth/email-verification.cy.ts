...
describe('email verification', { defaultCommandTimeout: 60000 }, () => {
  ...
  it('should signup activate account and login', () => {
    ...
    cy.get('app-signup [formControlName="email"]').type('e2e-user99@localhost');
    cy.get('app-signup [formControlName="username"]').type('e2e-user99');
    ...
    //     expect(activationUrl).to.contain('e2e-user99@localhost');
      ...
      .then((subject) => {
        return firstValueFrom(
          subject.pipe(
            ...
            filter((message) => message?.Content?.Body?.includes('e2e-user99@localhost')),
            first(),
    ...
    activateAccount('e2e-user99-password');
    login('e2e-user99', 'e2e-user99-password');
  });
